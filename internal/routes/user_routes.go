...
func SetupUserRoutes(mux *http.ServeMux, handler *handlers.Handler) {
	...
	mux.HandleFunc("GET /api/users/session", middlewares.AuthMiddleware(http.HandlerFunc(handler.GetUserByIdHandler())))
	mux.HandleFunc("PATCH /api/users/{id}", handler.UpdateUserHandler())
	mux.HandleFunc("DELETE /api/users/{id}", handler.DeleteUserHandler())
	mux.Handle("POST /api/users/session/logout", middlewares.AuthMiddleware(http.HandlerFunc(handler.LogoutUserHandler())))
